...
export async function copyChannelOverrideSpreadsheetToApiV3(inEvent: S3CreateEvent): Promise<void> {
    ...
    const now = new Date();
    const datePath = `${now.getUTCFullYear()}/${now.getUTCMonth().toString().padStart(2, '0')}/${now
        .getUTCDate()
        .toString()
        .padStart(2, '0')}`;
