...
import {
    FanaticsFanoutError,
    fanoutIfLargeSpreadsheetAndFanatics,
    getFanaticsAccountForEnv,
    sendFanaticsEmail
} from '@lib/fanatics';
import {
...
export async function publishCategorySpreadsheet(inEvent: S3CreateEvent | PublishCategorySpreadsheetEvent): Promise<void> {
    ...
    try {
        ...
        await sendFanaticsEmail(event, {genericMessage: 'message' in error ? error.message : `Unexpected error: ${JSON.stringify(error)}`});
        await sendWebsocketEvent(
            'error',
            {
                error,
...
async function publishSpreadsheetImpl(event: PublishCategorySpreadsheetEvent): Promise<Result<CatalogSpreadsheetWebsocketEvents['success'], UnexpectedError>> {
    const {
        categoryPath,
        retailerId,
        supplierId,
    ...
    for await (const resolvedBatch of gearmanCalls) {
        for (const resolvedBatchError of resolvedBatch) {
            if (resolvedBatchError) {
                await sendFanaticsEmail(event, {rowWithError: resolvedBatchError.rowIdx, validationErrors: resolvedBatchError.messages});
