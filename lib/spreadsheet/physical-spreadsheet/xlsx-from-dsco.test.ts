...
import { getCellData, getValidationErrorsForAColumnFromCatalogData } from './xlsx-from-dsco';

const RETAILER_ID = 1234;
const SUPPLIER_ID = 1235;
const BLANK_CELL: CellObject = { t: 'z' };

describe('Convert dsco spreadsheet to xlsx file', () => {
    describe('Get cell data from catalog data', () => {
        test('Undefined core value results in empty cell', () => {
            ...
            const testColumn = new DscoColumn('cost', 'test description only', 'core', {
                required: PipelineErrorType.error,
                format: 'number',
                min: 0,
            });
            const expectedResult: CellObject = { t: 'z' };
            const testResult = getCellData(testCatalogData, testColumn, RETAILER_ID);
            expect(testResult).toEqual(expectedResult);
        });

        test('Will not read .length attribute if extended_attributes is an array', () => {
            const testCatalogData: CoreCatalog = {
                supplier_id: SUPPLIER_ID,
                categories: {},
                extended_attributes: {
            ...
            const testColumn = new DscoColumn('length', 'test description only', 'extended', {
    ...
    describe('Extract error messages from compliance data in CoreCatalog object', () => {
